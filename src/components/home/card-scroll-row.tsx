...
export function CardScrollRow({ children, lang, className = '' }: { children: ReactNode; lang: Locale; className?: string }) {
  ...
  return (
    <div className="relative">
      ...
      <button type="button" aria-label={label} title={label} className="absolute right-0 lg:-right-4 top-1/2 grid h-10 w-4 -translate-y-1/2 place-items-center text-black" onClick={() => {
        const element = row.current
