...
export const Route = createFileRoute('/$locale/platform/psp/$gameId')({
  ...
  head: ({ loaderData, params }) => {
    ...
    const title = english ? `${loaderData.title} | PSP Game Information | Retro Game Hall` : `${loaderData.title}｜PSP游戏资料与截图｜怀旧游戏厅`
    const description = english
      ? `View ${loaderData.title} for PSP, including screenshots, publisher, release date, language and version information.`
      : `查看《${loaderData.title}》PSP游戏资料，包括游戏截图、厂商、发行日期、语言与版本信息。`
    return {
      links: getLocalizedSeoLinks({ locale, origin: SITE_ORIGIN, path: `/platform/psp/${params.gameId}` }),
    ...
    <SiteLayout locale={lang} hideFooter>
      <main className="min-h-screen bg-base-200 px-3 py-5 sm:px-6 lg:px-8">
        <article className="mx-auto max-w-6xl">
          {!isProStandalone ? <Link className="mb-4 inline-flex items-center gap-1 text-sm text-base-content/65" params={{ locale: lang, platformId: 'psp' }} to="/$locale/platform/$platformId">← {copy.back}</Link> : null}
          <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
            ...
            <aside className="rounded-xl bg-base-100 p-5 shadow-sm">
              ...
              <GameFavoriteButton className="mt-4 w-full" cover={game.cover} gameId={game.id} locale={lang} name={game.title} platform="PSP" />
            </aside>
